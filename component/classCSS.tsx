export const buttonCSS = "rounded py-1 px-3 bg-indigo-600 text-white hover:bg-indigo-700"

export const h2CSS= "text-xl"

export const modalCSS ="fixed inset-0 bg-black/50 flex items-center justify-center  z-10"

export const dialogCSS ="bg-white  w-150 h-100 overflow-hidden border-4 border-mist-500 rounded-3xl shadow-xl p-2 focus:outline-none"

export const w_A3pa = " w-398 "

export const h_A3pa =" h-281 "

export const w_A4pa = " w-281.25 "

export const h_A4pa =" h-198.5 "

export const A3pa =w_A3pa+h_A3pa

export const A4pa =w_A4pa+h_A4pa


export const small= " w-66 h-42.5 "

export const smallpo= " w-42.5 h-66 "